...
// Helper: resolve and validate priority, defaulting to P3 when absent
function resolvePriority(priority) {
  if (priority === undefined || priority === null || priority === '') {
    return { value: 'P3' };
  }
  if (!VALID_PRIORITIES.includes(priority)) {
    return { error: 'Priority must be one of P1, P2, P3' };
  }
...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    if (resolvedPriority.error) {
      return res.status(400).json({ error: resolvedPriority.error });
    }
    const stmt = db.prepare('INSERT INTO tasks (title, description, due_date, priority) VALUES (?, ?, ?, ?)');
    const result = stmt.run(title, description || '', due_date || null, resolvedPriority.value);
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    const result = stmt.run(title, description || '', due_date || null, resolvedPriority.value, req.params.id);
